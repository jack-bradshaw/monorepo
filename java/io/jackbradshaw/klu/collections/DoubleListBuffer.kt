 ...
 *
 * This can be for batch processing where a stream of values needs to be buffered and processed as a batch while new
 * entries continue to be produced for processing in the next batch. For example:
 *
 * // In one thread
 * fun accept(someItem: Item) {
 *   buffer.getActive().add(someItem)
 * }
 *
 * // In another thread
 * fun process() {
