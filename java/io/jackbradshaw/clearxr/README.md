...
- Tutorial: How to use the library in your project.
...
These issues are well known errors in application architecture and they should be avoided wherever possible. clearxr
...
This section covers how to use the clearxr library in your application. It's divided into:

- Getting access: How to actually import the library into your project.
- Core usage guide: How to use the core features of the library.
- Advanced usage guide: How to use the advanced features of the library.

If you have questions after reading the tutorial please email jack@jackbradshaw.io.
...
The pre-built binaries are available via Maven and can be accessed at `io.jackbradshaw:clearxr:0.0.1`. Visit
the [Maven Central Repository](https://search.maven.org/artifact/io.jackbradshaw/clearxr) previous versions.
...
```
import io.jackbradshaw.clearxr.config.manifestFile
...
Once you have a config you need to instantiate the clearxr object to get access to the various utilities. It can be
instantiated by calling the factory directly:

```
import io.jackbradshaw.clearxr.config.config
import io.jackbradshaw.clearxr.Daggerclearxr
import io.jackbradshaw.clearxr.clearxr

val clearxr: clearxr = Daggerclearxr.create(config("/home/myapp", "action_manifest.json"))
```
...
val clearxr: clearxr = clearxr(config("/home/myapp", "action_manifest.json"))
```

From here you can access the various functions of the library, for example:

```
val clearxr = /* same as above */
clearxr.manifestInstaller()
```

You may have noticed that the clearxr object is a Dagger Component. If you're using Dagger in your application you could
include clearxr in your component dependencies to include the bindings in your graph. This would let you inject the
...
class MyClass @Inject internal constructor(
  val clearxrInstaller: ManifestInstaller
)
```
...
val clearxr = clearxr(config("/home/myapp", "action_manifest.proto"))
...
the same for all of them. Install the manifest with clearxr, register the manifest with the framework, observe the
