 ...
 */
interface SceneStage : SceneStageProperties, SceneStageServices, SceneStageMutations, SceneStageEvents
...
interface SceneStageEvents {
  /**
   * A flow which emits elements as they are attached to the stage. Ordering is not guaranteed to be consistent.
   */
  @ColdFlow
  @IndefiniteFlow
  fun elementEnteredScene(): Flow<ScenePrimitive>

  /**
   * A flow which emits elements as they are detached from the stage. Ordering is not guaranteed to be consistent.
   */
  @ColdFlow
  @IndefiniteFlow
  fun elementExitedScene(): Flow<ScenePrimitive>

  /**
   * A flow which emits props as they are attached to the stage (directly or transitively via another item). Ordering is
   * not guaranteed to be consistent.
   */
  @ColdFlow
  @IndefiniteFlow
  fun itemEnteredScene(): Flow<SceneItem>

  /**
   * A flow which emits props as they are detached from the stage (directly or transitively via another item). Ordering
   * is not guaranteed to be consistent.
   */
  @ColdFlow
  @IndefiniteFlow
  fun itemExitedScene(): Flow<SceneItem>
}