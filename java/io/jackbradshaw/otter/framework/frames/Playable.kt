...
interface Playable {
   ...
   *
   * A steady state is one which will undergo no further simulation until disturbed. Examples
   * include a pendulum hanging straight downwards with no momentum, a box which has landed on solid
   * ground, and a player with no current controller inputs. Small amounts of noise at steady state
   * are tolerable and should not result in false being emitted.
   */
