...
/** Abstract tests that all [Outlet] instances should pass. */
@RunWith(JUnit4::class)
import com.jackbradshaw.sealant.connectable.ConnectableTest

abstract class OutletTest<T, O : Outlet<T>> : ConnectableTest<O>() {
  ...
  fun beforeClose_flowCollectedWhileAlreadyCollected_fails() =
      runBlocking<Unit> {
        ...
        assertThat(exception)
            .hasMessageThat()
            .isEqualTo(
                "Outlet flows can only be collected by a single downstream consumer. They cannot be collected repeatedly, even if the previous collector has disconnected.")
      }
  ...
  protected abstract fun subject(): O
