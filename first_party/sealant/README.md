...
1. One or more downstream pipelines that must process every emission.
1. Absolutely no room for emissions being dropped by late subscribers or missed by downstream
   pipelines.
...
"sealed session", which is designed to reliably produce an `isConnectedToSource` signal when the of
three primary constructs: SealedSource, SealedHub, and SealedSession.

A sealed source acts as the origin point of a pipeline where data is manually emitted. A sealed hub
takes an existing flow and acts as an intermediate upstream for further sealed sessions. Both
produce one or more `SealedSession` instances, each representing a pipeline coming off the origin.
They receive values and forward them to each sealed session for downstream processing. The critical
factor that makes this more than a shared flow is the observability: Every sealed session exposes an
`isConnectedToSource` state flow that indicates whether its final downstream collector is actively
listening AND whether its internal pipeline is securely connected to the upstream source/hub.

Crucially, the `SealedSession` encapsulates any flow transformations (e.g., map, filter) you wish to
apply. Because these transformations occur _inside_ the sealed pipeline, they are monitored, meaning
you can guarantee `collect` is actually running from the very top to the very bottom.

Flow diagram:

```text
source flow -> sealed hub -> sealed session (containing operators) -> collector
                     |
                     |------> sealed session (containing operators) -> collector
                     |
                     |------> sealed session (containing operators) -> collector
```
...
val logSession = hub.createSession()
val checkSession = hub.createSession()
val processingSession = hub.createSession()
...
`sealedSession.isConnectedToSource.value`.

## Advanced Usage

Sealant can only prevent leaks within the constructed pipeline, and it cannot detect or prevent
leaks upstream of the root hub or downstream of the final `SealedSession` output; therefore the hub
should be defined as close to the upstream source as possible, and the `SealedSession` output should
be collected as directly as possible by the downstream consumer. While transformations can be safely
passed into `createSession`, complex architectures where flows pass through many layers of
abstraction and indirection might benefit from using multiple cascading hubs/sessions across the
pipeline to reduce the overall risk of leakage. Below is an example of cascading hubs:

```kotlin
// Layer 1

class Layer1(private val sealedSourceFactory: SealedSource.Factory) {

  private val coroutineScope = CoroutineScope(Dispatchers.IO)

  private val dataSource = DataSource()

  private lateinit var sealedSource: SealedSource<String>

  suspend fun initialize() {
    sealedSource = sealedSourceFactory.create<String>()
  }

  /** Emits data from the upstream. */
  suspend fun observeData(): SealedSession<String> = sealedSource.createSession()

  suspend fun startEmissions() {
    coroutineScope.launch {
      dataSource.onNewData {
        sealedSource.emit(it)
      }
    }
  }
  ...
  fun close() {
    ...
    if (::sealedSource.isInitialized) sealedSource.close()
  }
}
...
class Layer2(
  ...
  suspend fun observeDataCount(): SealedSession<Int> = emissionCountHub.createSession()

  suspend fun observeHelloWorldSeen(): SealedSession<Boolean> = helloWorldSeenHub.createSession()
...
class Layer3(
  ...
  suspend fun logDataCount() {
    val emissionCountSession = layer2.observeDataCount()
    val helloWorldSeenSession = layer2.observeHelloWorldSeen()

    coroutineScope.launch {
      emissionCountSession.flow.onEach { totalCount ->
        println("Received another value, bringing total count to $totalCount")
      }.collect()
    }

    coroutineScope.launch {
      helloWorldSeenSession.flow.filter { it }.onEach {
        println("hello world went down the pipe")
      }.collect()
    }

    emissionCountSession.awaitConnectionToSource()
    helloWorldSeenSession.awaitConnectionToSource()
...
1. `SealedHub` is usually a private implementation detail. It collects data from upstream and
   provides sealed sessions when the class is used. Multiple hubs can be required when a class
   provides multiple distinct sessions to keep them separate, though you can also use a single hub
   and provide different transformations via `createSession { ... }`.
1. `SealedSession` is usually a public API detail. It declares the leak-proof nature of the session
   which allows consumers to check connection status and continue to create leak-proof sessions
   downstream. A new `SealedSession` is provisioned by the hub each time the session is requested to
   avoid illegal double usage.
...
By passing transformations directly into `createSession { flow -> ... }`, users can safely apply
operators (even "leaky" ones like `buffer`) knowing they are fully monitored by the internal
pipeline.

## Closure

SealedHub and SealedSession are active systems that operate and coordinate coroutine jobs
...
class CascadingPipeline(private val sealedHubFactory: SealedHub.Factory) {
  ...
  suspend fun startDownstream() {
    ...
    val topOfPipeSession = topOfPipeHub.createSession()

    // Lifecycle is intentionally linked to topOfPipeSession since with-automatic-closure is used.
    hub1 = sealedHubFactory.createWithAutomaticClosure(
      underlyingFlow = topOfPipeSession
    )

    // Lifecycle is NOT linked to topOfPipeSession since with-automatic-closure was not used.
    hub2 = sealedHubFactory.create(
      underlyingFlow = topOfPipeSession.flow
    )

    // Lifecycle is inherently linked to hub1 because all sessions are linked to their hub.
    val hub1Session = hub1.createSession()
    coroutineScope.launch {
      hub1Session.flow.collect { println(it) }
    }

    // Lifecycle is inherently linked to hub2 because all sessions are linked to their hub.
    val hub2Session = hub2.createSession()
    coroutineScope.launch {
      hub2Session.flow.collect { println(it) }
    }
  }
  ...
  suspend fun close() {
    ...
    // Closes topOfPipeHub -> closes topOfPipeSession -> closes hub1 -> closes hub1Session
    // Does NOT close hub2 or hub2Session because they were created without automatic closure.
    if (::topOfPipeHub.isInitialized) topOfPipeHub.close()

    // Closes hub2 -> closes hub2Session
...
transformations are contained in the `createSession` function of a hub, and each hub is linked
together without further transformations. For example, there are three leaks in the following code:
...
class LeakySystem(
  ...
  suspend fun initialize() {
    ...
    val sealedSession1 = flowHub1.createSession()
    val leakyIntermediate = sealedSession1.flow.buffer(capacity = 3)
    flowHub2 = sealedHubFactory.create(leakyIntermediate)

    coroutineScope.launch {
      // Not passing the transformation into createSession()!
      val sealedSession2 = flowHub2.createSession()

      launch {
        // LEAK 3: Applying a buffer AFTER the sealed session.
        // The connection after sealedSession2 is broken by the buffer, similar to leak1.
        sealedSession2.flow
            .map { it.length }
            .buffer(capacity = 3)
            .filter { it > 2 }
      ...
      sealedSession2.awaitConnectionToSource()
...
class ResilientSystem(
  ...
  suspend fun initialize() {
    ...
    val sealedSession1 = flowHub1.createSession { it.buffer(capacity = 3) }
    flowHub2 = sealedHubFactory.create(sealedSession1.flow)

    coroutineScope.launch {
      // FIX 3: Encapsulate all downstream transformations inside createSession!
      val sealedSession2 = flowHub2.createSession { flow ->
          flow.map { it.length }.buffer(capacity = 3).filter { it > 2 }
      }

      launch {
        sealedSession2.flow.onEach {
          println("Observed string of length over 2")
        }.collect()
      }

      // Wait for both pipelines to be securely connected from bottom to top!
      sealedSession1.awaitConnectionToSource()
      sealedSession2.awaitConnectionToSource()
...
By ensuring your code follows this approach by keeping your transformations inside `createSession`,
Sealant can guarantee the entire pipeline is connected from the first source to the final collector
before `awaitConnectionToSource()` resumes, thus eliminating race conditions even across
asynchronous boundaries and complex intermediate operators.

## Dagger

Dagger is used to manage dependencies and provide instances of `SealedHub.Factory` and
`SealedSource.Factory`. You can get instances from `SealantComponent` (and its implementation,
`SealantComponentImpl`) directly, or depend on it to inject them through your code. The
`sealantComponent` helper function makes it easy to instantiate with sensible defaults. For example:
...
`SealantComponent` depends on `CoroutinesComponent` from the
[coroutines package](/first_party/coroutines). By default it uses coroutines that are well suited
for flow-based work, but if you need granular control over coroutines, an alternative can be passed
in like so:
...
testing of asynchronous code, which is particularly useful when dealing with flows. View the package
for more details.
...
both time and space overhead. This is the cost of leak-proof flows, and using sealant is a tradeoff
between performance and correctness. If you require a pipeline where data does not begin to flow
until the pipeline is definitively setup, use Sealant. If lost emissions are recoverable or
unimportant, Sealant is unnecessary.
