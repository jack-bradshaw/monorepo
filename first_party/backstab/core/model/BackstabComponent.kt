 ...
 */
data class BackstabComponent(val packageName: String, val name: String, val builder: Builder?) {
  /** Represents the @Component.Builder or @Component.Factory interface. */
  data class Builder(val bindings: List<BuilderMethod>)
  ...
  data class BuilderMethod(
      ...
      /**
       * The @Named qualifier, if present. Dagger treats @Named specially, so we promote it to a
