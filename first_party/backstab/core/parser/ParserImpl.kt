...
class ParserImpl : Parser {
  ...
  override fun parseModel(component: KSClassDeclaration): BackstabComponent? {
    ...
    return BackstabComponent(packageName = packageName, name = componentName, builder = builder)
  }

  private fun findBuilder(component: KSClassDeclaration): BackstabComponent.Builder? {
    val builder =
        component.declarations.filterIsInstance<KSClassDeclaration>().firstOrNull {
    ...
    builder.getAllFunctions().forEach { func ->
      ...
      func.parameters.forEach { param ->
                ...
                .filter { ksAnnot ->
                  ...
                  declaration.annotations.any { metaAnnot ->
                    metaAnnot.shortName.asString() == "Qualifier" &&
                        metaAnnot.annotationType.resolve().declaration.packageName.asString() ==
                            "javax.inject"
                  }
                }
                .map { ksAnnotation ->
        ...
        val namedAnnotation =
            extractedQualifiers.firstOrNull { it.typeName.toString() == "javax.inject.Named" }

        val otherQualifiers =
            extractedQualifiers.filter { it.typeName.toString() != "javax.inject.Named" }
