...
class GeneratorImpl @Inject constructor() : Generator {
  ...
  /**
   * Generates the source code for a backstab module that provides [target] and uses [metadata] for
   * package/name information. The generated module accurately targets the factory, builder or
   ...
   * create-function of the Dagger-generated component associated with [target].
   */
   ...
   * Dagger-generated component associated with [target]. No check is performed to verify the
  ...
  private fun FunSpec.Builder.configureForCreateInstantiator(target: BackstabTarget) {
    addStatement("return %T.create()", target.generatedComponentClassName())
  }
  ...
  private fun FunSpec.Builder.configureForBuilderInstantiator(
    ...
    val statement = StringBuilder("return %T.builder()")
  ...
  /**
   * Configures this spec with code that instantiates [target] via the [instantiator] factory. No
   * check is performed to verify the factory actually exists, so the caller must verify this is the
   * correct instantiation approach for [target]. This function mutates the builder directly so
   * nothing is returned.
   */
  private fun FunSpec.Builder.configureForFactoryInstantiator(
    ...
    addStatement(
        "return %T.factory().%L(${paramNames.joinToString()})",
