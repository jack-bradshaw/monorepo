...
class BackstabApplication : Application {
  ...
  override suspend fun onCreate(component: Application.ContextComponent) {
    ...
    mainJob =
        coroutines.ioCoroutineScope().launch {
          val hostComponent =
              DaggerHostComponentImpl.builder().consuming(component).consuming(coroutines).build()

          launch { hostComponent.kspRepository().run() }

          DaggerCoreComponentImpl.builder().consuming(hostComponent).build().main().run()
        }
  }
...
@CoreScope
@Component(
    dependencies = [Application.ContextComponent::class, Coroutines::class],
...
interface HostComponentImpl : HostComponent {
  ...
  @Component.Builder
  interface Builder {
    fun consuming(applicationComponent: Application.ContextComponent): Builder
