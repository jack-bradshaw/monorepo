...
class BackstabTest {
  ...
  /**
   * Verifies that the simplest component (Leaf) generates a valid module. The LeafComponent has no
   * bindings and no builder arguments. Use Case: Base components in a graph.
   */
  @Test
  fun leafComponent() {
  ...
  /**
   * Verifies that a single @BindsInstance argument in the builder is correctly propagated to the
   * generated module's provider method.
   */
  @Test
  fun singleBindingComponent() {
    assertThat(
            DaggerSingleBindingMetaComponent.builder().foo(Foo()).build().singleBindingComponent())
        .isNotNull()
  }

  /**
   * Verifies multiple bindings are correctly handled. Ensures order and type matching of multiple
   * arguments.
   */
  @Test
  ...
  /**
   * Verifies that Qualifier annotations (e.g. @Named) are preserved. Dagger relies on these to
   * distinguish bindings of the same type.
   */
  @Test
  ...
  /**
   * Verifies Component Dependencies. The generated module should request the dependency component
   * (ProviderComponent) as a parameter to the provider method.
   */
  @Test
  ...
  /**
   * Verifies mixing Component Dependencies with Manual Bindings. The generated provider should
   * accept both the component dependency and the bound instance.
   */
  @Test
  ...
  /**
   * Verifies multiple Component Dependencies. The component depends on both ProviderComponent and
   * LeafComponent. Both should be requested by the generated module.
   */
  @Test
  ...
  /**
   * Verifies a deep dependency graph (Bottom -> Middle -> Top). Ensures that Backstab generated
   * modules work correctly when participating in a transitive chain.
   */
  @Test
  ...
  /**
   * Verifies a Diamond Graph structure to ensure deduplication. Structure: Source -> [Left, Right]
   * -> Shared. We create one `Shared` instance. Both `Left` and `Right` depend on it. `Source`
   * depends on both `Left` and `Right`. We verify that the `Source` component sees the *same*
   * `Shared` instance through both paths. This confirms that Backstab didn't inadvertently
   * duplicate the graph or create new instances.
   */
  @Test
  fun diamondGraph_equalityCheck() {
     ...
     */
    val meta =
        DaggerDiamondSourceMetaComponent.builder()
    ...
    // We can't access `left` or `right` from `source` unless `DiamondSourceComponent` interface has
    // getters for them.
    ...
    // But to verify *equality* (that it's the SAME instance), we strictly need to extract `Shared`
    // from them.
    ...
    // The real test is: Does generated `DiamondSourceComponent` use the provided `left` and
    // `right`?
    ...
    // The "Equality Check" requested by the user implies we should verify the graph isn't
    // duplicated.
