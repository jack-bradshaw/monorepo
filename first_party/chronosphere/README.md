...
1. The test needs to wait for the system to finish its work.

The former is called advancement, and the latter is called idling.

Advancement is usually required when a test cannot make an assertion or move on to the next step
until the system under test has been artificially driven through a specific duration. A common
example in UI programming is animations: A screendiff test might need to drive the UI through 50
milliseconds of animations and screenshot the UI to ensure the animation looks right. In such cases,
the difficulty is not waiting for a complex multithreaded environment to complete, it's driving the
underlying system forward in time with granular control. This approach to testing, called
Advancement, generally relates to single-threaded systems, since synchronizing virtual time across
multiple threads is difficult and not well defined, but multi-threaded systems can be advanced in
some scenarios depending on how they are architected.

Idling is usually required when a test cannot make an assertion or move on to the next step until
the system under test has naturally finished its work. A common example in front-end programming is
disk read/write events: For performance, such operations usually occur on a background thread, with
an asynchronous mechanism (such as callbacks) used to notify the main thread when the task is
complete. In such cases, the difficulty is not driving the system through a fixed amount of time,
but pausing the test until all background work has finished. This approach to testing, called
Idling, generally relates to multi-threaded systems, but single-threaded systems can be idled in
some scenarios depending on how they are architected.
...
Alternatively, you could wait for fixed periods in your tests using `Thread.sleep()` or coroutine
...
The solution that avoids production API pollution and hardcoded waits is to replace the underlying
...
`TestingTaskDriver`. Below is a complete example that demonstrates tool usage with instantiation
handled by Dagger.
...
the `TestingTaskBarrier`. Below is a complete example that demonstrates tool usage with
instantiation handled by Dagger.
...
This is useful if multiple unrelated libraries expose `TestingTaskBarrier`/`TestingTaskDriver`
objects. If you could not combine them together, you would need to manually loop over them, which
adds boilerplate to your tests. Composing them saves work and is less error-prone.

## Concurrency

The various APIs throughout Chronosphere use blocking functions instead of suspending functions
because Chronosphere exists to make concurrent systems testable, and requiring a coroutine context
at the usage site would defeat the purpose by introducing yet another layer of concurrency.
...
`Idleable` implementations and be composable with any other `TestingTaskBarrier`. For convenience,
the `TestingTaskBarrier` and `TestingTaskDriver` tests are abstract contract tests, and you can
...
The [CoroutinesComponent](/first_party/coroutines/CoroutinesComponent.kt) is integrated with
Chronosphere. It provides standard dispatchers that can be advanced/idled in tests. It can be used
and provides helpful examples for extending Chronosphere to other systems.
