...
abstract class TestingTaskDriverTest {
  ...
  @Test
  fun advanceBy_withNoAdvancables_returnsSuccessfully() = runBlocking {
    setupSubject(emptyList())
    val driver = subject()
  ...
  @Test
  fun advanceBy_withMultipleAdvancables_advancesEveryVirtualClockUniformly() = runBlocking {
    val advancables = List(5) { ObservableAdvancable() }
    setupSubject(advancables)
    val driver = subject()
  ...
  /**
   * An Advancable for use in tests.
   *
   * Cumulative time can be observed by reading [totalTime].
   */
