...
class ResourceManagerImpl<K, V : ObservableClosable>(coroutineContext: CoroutineContext) :
  ...
  override fun close() {
    val (jobsToCancel, itemsToClose) =
        runBlocking {
          lock.withLock {
            if (hasTerminalState.value) return@withLock Pair(emptyList<Job>(), emptyList<V>())
    ...
    runBlocking { jobsToCancel.forEach { it.cancelAndJoin() } }
  ...
  private inner class OperatorImpl : ResourceManager.Operator<K, V>, AutoCloseable {
    ...
    override suspend fun getOrPut(key: K, newValueProvider: () -> V): V {
      return lock.withLock {
        ...
        observeTerminationJobs.remove(key)?.cancel()
        observeTerminationJobs[key] = coroutineScope.launch { newResource.removeSelfOnClosure(key) }
  ...
  internal constructor(@Cpu private val coroutineContext: CoroutineContext) :
    ...
    override fun <K, V : ObservableClosable> createResourceManager():
        ResourceManager<K, V> = ResourceManagerImpl(coroutineContext)
  }
}
