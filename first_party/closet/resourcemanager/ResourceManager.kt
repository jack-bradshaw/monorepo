 ...
 * 
 * 1. Registration: Associating a resource with this manager so it can inherit the manager's
 *    closure. Functions are [put] and [getOrPut].
 * 2. Deregistration: Dissociating a resource from this manager so it no longer inherits the
 *    manager's closure. Functions are [remove] and [clear].
 ...
 * accessor/mutator calls are required in succession without yielding access to other threads. It
 * effectively provides mutex-like access to the manager and causes all other accessor/mutator
 * functions to suspend until it exits.
 *
 * Caveats:
 * 
 ...
 * 3. Closing a registered resource externally (i.e. closed by means unrelated to the manager) will
 *    cause the manager to automatically deregister it; however, external resources closure can be
 *    triggered any time by any thread, so there could be a delay between external closure and
 *    automatic deregistration; therefore, functions such as [get], [getOrPut], [remove], and
 *    [clear], cannot guarantee the returned value(s) are open, and callers should check their
 *    status directly when a particular status is strictly necessary. Such cases are handled
 *    gracefully by the manager and do not cause internal failures.
 * 4. Attempting to register an already closed resource results in an exception.
 * 5. Attempting to call any mutator/accessor function after the manager has been closed results in
 *    an exception.
 * 6. All accessor/mutator functions are thread-safe.
 * 7. All calls to accessor/mutator functions suspend while an [exclusiveAccess] block is being
 *    evaluated.
 */
interface ResourceManager<K, V : ObservableClosable> : ObservableClosable {
   ...
   * unrelated to the manager), and this function checks to ensure the resources are open before
   * returning them; however, an unavoidable race condition exists: A resource could be externally
   * closed after being checked but before being returned; therefore, callers should verify the
   * status of the returned resource when a particular state is strictly necessary, and not assume
   * the returned resource is open.
   */
  suspend fun get(key: K): V?

  /**
   * Registers [resource] and associates it with [key]. If another resource is already associated
  ...
  /**
   * Returns the registered resource associated with [key]. If none exists, evaluates
   * [newValueProvider], registers the result, associates it with [key], and returns it.
   *
   * Suspends until exclusive access to the manager can be guaranteed. Throws
   ...
   * returning them; however, an unavoidable race condition exists: A resource could be externally
   * closed after being checked but before being returned; therefore, callers should verify the
   * status of the returned resources when a particular state is strictly necessary, and not assume
   * the returned resources are open.
   */
  ...
  interface Operator<K, V> {
     ...
     * returning them; however, an unavoidable race condition exists: A resource could be externally
     * closed after being checked but before being returned; therefore, callers should verify the
     * status of the returned resources when a particular state is strictly necessary, and not
     * assume the returned resources are open.
     */
    suspend fun clear(): List<V>
