...
- [ObservableClosable](/first_party/closet/obsevable/ObservableClosable.kt), an interface for
...
- [ClosetRule](/first_party/closet/rule/ClosetRule.kt), a JUnit test rule for automatic closing
...
`ObservableClosable` interface. It represents a closable that broadcasts its closure status with two flags:
...
2. `hasTerminatedProcesses`, which indicates the termination of any background work the closable
...
class ConnectionCoordinator @Inject constructor(
  ...
  suspend fun openConnection(destination: String) {
    ...
    // Implicit network initialization
  }
