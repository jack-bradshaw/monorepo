...
Editorial directives for the [journal documents](/first_party/site/static/content/journal) in this
package.
...
Punctuation must be contained with the quotation marks only if it is part of the original quoted
material; whereas, any punctuation used to structure the surrounding sentence must be placed outside
the quotation marks.
...
unless it modifies the presentation/meaning of the dialogue is permitted (e.g. question marks,
exclamation marks, ellipses, etc.). Regardless of the presence or absence of final punctuation in
the quoted dialogue, a comma must be placed after the closing quotation mark.
