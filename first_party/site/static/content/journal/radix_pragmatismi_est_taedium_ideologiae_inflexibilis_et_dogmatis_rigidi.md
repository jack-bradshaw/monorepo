...
multiple build files). If there are occasions when rules do not apply though, then what are we to do
when our well-intentioned peers argue for them to an unreasonable degree? Moreover what do we do
...
(readability and test nature), but I suppose, seeking balance between pragmatism and idealism
...
automated tests, and I would agree with this position, given infinite time; however, unfortunately
nature is not based on adherence to such principles, and occasionally, our principles are in
conflict with our survival.
...
long-term, the nature of an entropic universe dictates a more nuanced approach in emergencies. We
live in a world where the mistake, the unexpected, and the undesirable all occur around us without
our consent, and we must adapt to that reality. We can use principle to mitigate these forces as
best we can, and when applied consistently they compound to work effectively, but we cannot achieve
perfection, and when the hull buckles through no fault of our own, a temporary patch is better than
a sinking ship. Less poetically, keeping production green is more important than checking
theoretical boxes, and we must occasionally compromise our desire for perfection to sustain
ourselves in the long-term. We should of course be wary of those who would argue for short-term
gains without justification, as greed is far inferior to both idealism and pragmatism, but when we
are forced to make hard decisions through no fault of our own, engineers should choose simple
...
down on us with entropy is untenable. We must prioritise pragmatism over idealism when intractable
conflict arises, and remember that engineering does not exist to serve abstract principles; quite
the opposite: ideals are merely tools engineers use to solve problems, build systems, and manage
...
are the root of all pragmatism.
