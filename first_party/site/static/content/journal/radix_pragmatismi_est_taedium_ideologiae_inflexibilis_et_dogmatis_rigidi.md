...
down on us with entropy is untenable. We must prioritize pragmatism over idealism when conflict
arises, and remember that engineering does not exist to satisfy abstract principles, instead it exists to solve problems. If we could eliminate entropy then it might be a different story, but no engineer has ever launched that to production, so for now we will just have to make do with the occasional compromise.
