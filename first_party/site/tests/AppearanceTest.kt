...
class AppearanceTest {
   ...
   *
   * Opens [path] in a browser of width [size], captures a screenshot, and compares it to the golden
  ...
  /** Captures a screenshot of [page] and compares it to the golden screenshot. */
  private fun performScreendiff(page: Page) {
    page.waitForLoadState(LoadState.NETWORKIDLE)
    page.evalOnSelectorAll(
        "img", "images => Promise.all(images.map(img => img.decode().catch(() => {})))")
  ...
  /**
   * Saves [screenshot] to the during-test output directory, with a filename derived from
   * [goldenName], and returns the full path to the file (in the post-test output directory).
   */
  private fun saveScreenshot(screenshot: ByteArray, goldenName: String): Path {
    val path = Paths.get(DURING_TEST_SCREENSHOT_OUTPUT_LOCATION, goldenName)
    path.toFile().writeBytes(screenshot)

    return Paths.get("${POST_TEST_SCREENSHOT_OUTPUT_LOCATION}/appearance_test/shard_${System.getenv("TEST_SHARD_INDEX")}_of_${System.getenv("TEST_TOTAL_SHARDS")}/test.outputs/$goldenName")
  }

  companion object {
    /** The directory for saving artefacts during test operation */
    private val DURING_TEST_SCREENSHOT_OUTPUT_LOCATION =
        System.getenv("TEST_UNDECLARED_OUTPUTS_DIR").also {
          check(it != null) { "TEST_UNDECLARED_OUTPUTS_DIR environmental variable not set." }
        }

    /**
     * The directory where the contents of [DURING_TEST_SCREENSHOT_OUTPUT_LOCATION] are moved to
     * after the test ends.
     */
    private const val POST_TEST_SCREENSHOT_OUTPUT_LOCATION =
        "bazel-testlogs/first_party/site/tests/"

    /** The directory containing screendiff goldens, relative to the runfiles root. */
