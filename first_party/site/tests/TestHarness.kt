...
class TestHarness {
  ...
  /** Used to access files from the build. */
  ...
  /** The port the server is running on, assigned a meaningful value when [server] is started. */
  ...
  /** Opens a new browser page at [fragment], where [fragment] is relative to the site root. */
  fun openPage(fragment: URI): Page =
      browserContext.newPage().apply { navigate(endpoint().resolve(fragment).toString()) }

  /** Returns the runfile identified by [path], where [path] is relative to the runfiles root. */
  ...
  /**
   * Finds the first element matching the [locator], scrolls it into view (if necessary), and
   * returns it.
   */
  fun findElement(locator: String): Locator =
      browserContext.pages().last().locator(locator).first().also { it.scrollIntoViewIfNeeded() }

  /** Initializes [runfiles] and loads the site from runfiles into [site]. */
  ...
  /** Initializes [server] with a random [port]. */
  ...
  /** Resolves the file relative to [site] that matches this request. */
  ...
  /** Releases the resources used by the instrumentation. */
  private fun tearDownInstrumentation() {
  ...
  companion object {
    ...
    /** The value which can be passed to [embeddedServer] to request a random port. */
...
/** The size of the screen horizontally. */
