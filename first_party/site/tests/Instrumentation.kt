 ...
 *
 * The [setup] function must be called before using the provided utilities, and the [tearDown]
 * function must be called when the harness is no longer needed.
 */
class Instrumentation {
  ...
  /**
   * Starts the server, sets the browser screen width to [screenWidth], and optionally sets the
   * cookie consent state.
   *
   * By default, [cookieConsent] is [CookieConsent.Set(true)], so that tests are not obstructed by
   * the privacy notice.
   */
  fun setup(
      screenWidth: ScreenWidth = ScreenWidth.MEDIUM,
      cookieConsent: CookieConsent = CookieConsent.Set(true)
  ) {
    setupSite()
    setupInstrumentation(screenWidth)
    setupServer()

    if (cookieConsent is CookieConsent.Set) setCookieConsent(cookieConsent.granted)
  }
  ...
  /** Opens [page] in a new window (relative to the site root). */
  ...
  /** Loads [site] from runfiles. */
  private fun setupSite() {
    site =
        "_main/first_party/site/site".toRunfile().toFile().also {
  ...
  private fun setupInstrumentation(width: ScreenWidth) {
    ...
    env["PLAYWRIGHT_BROWSERS_PATH"] =
        BROWSER_RUNFILES_PATH.toRunfile().toFile().parentFile.parentFile.absolutePath

    val browserExecutable = BROWSER_RUNFILES_PATH.toRunfile()
    ...
    val launchOptions =
        BrowserType.LaunchOptions()
            ...
            .setExecutablePath(browserExecutable)
  ...
  /** Sets the cookie consent state to [granted] in the data layer. */
  private fun setCookieConsent(granted: Boolean) {
    browserContext.addInitScript(
        "window.localStorage.setItem('analytics_consent_granted', '$granted');")
  }

  /** Resolves [this] path relative to the runfiles root. */
  private fun Path.toRunfile(): Path {
    val rlocation = runfiles.rlocation(toString())
    check(rlocation != null) { "Runfile not found: $this" }
    return Paths.get(rlocation)
  }

  /** Resolves [this] runfiles key to an absolute path. */
  private fun String.toRunfile(): Path {
    val rlocation = runfiles.rlocation(this)
    check(rlocation != null) { "Runfile not found for key: $this" }
    return Paths.get(rlocation)
  }
  ...
  companion object {
    ...
    private val BROWSER_RUNFILES_PATH =
        "rules_playwright++playwright+playwright/browsers/ubuntu24.04-x64/chromium_headless_shell-1148/chrome-linux/headless_shell"
  }
}
...
/** The cookie consent state for the site. */
sealed class CookieConsent {
  /** The user has not yet made a choice. */
  object NotSet : CookieConsent()

  /** The user has made a choice. */
  data class Set(val granted: Boolean) : CookieConsent()
}
