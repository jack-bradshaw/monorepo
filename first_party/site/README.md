...
-   Layouts ([/first_party/site/layouts](/first_party/site/layouts)): Defines the HTML structure and
    presentation logic. Layouts are responsible for integrating data and creating the final HTML pages.
...
-   Content Generators: Declare pages to display via the Hugo `AddPage` command. They do not
    populate the page with data directly; instead, they pass a `key` property that uniquely
    identifies the data associated with the page. Multiple generators exist, and Hugo evaluates each
    generator once at build time. The key property is optional and is omitted for pages that have no
    parameterization (e.g., the home page).
