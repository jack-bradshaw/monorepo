...
```bash
bazelisk run //first_party/formatting:autoformat -- /path/to/file/or/directory
```
...
Prettier matches ignore patterns in `.prettierignore` files relative to the directory containing the
ignore file. This has two issues:
...
2.  Runtime Synthesis. The runner script (`prettier_bin.sh`) reads these fragments from runfiles,
