...
main() {
  ...
  # Prettier modules (plugins) are located in the runfiles tree, but the next command moves to the
  # workspace root so they become inaccesible; therefore, the runfiles tree must be explicitly
  # added to the node path.
