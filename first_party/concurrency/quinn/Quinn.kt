 ...
 * single-argument suspending tasks that are supplied concurrently in one execution context (i.e.
 * the submission side) and execute sequentially in another (i.e. the execution side). Unlike the
 * conventional actor-object design pattern, Quinn is a reusable component that fully encapsulates
 * the entire process of queueing, dequeueing, and executing tasks, such that the submission side
 * can simply pass in tasks, and the execution side simply can simply supply the value to use when
 * running tasks. Furthermore, Quinn conflates the act of passing in the value with execution
 * itself, meaning when the execution side calls [execute] and passes in a value, the function
 * suspends indefinitely and processes the task queue. This ensures execution occurs in the context
 * that natively has access to the value.
 *
 * This single-arg task system is useful in situations where a thread-confined resource needs to be
 * used in other threads. Consider a main-threaded resource, perhaps a UI element, that cannot be
 * mutated by other threads. By constructing a Quinn and sharing it between the UI thread and
 * background worker threads, the workers can pass in tasks that operate on the object, and the UI
 * thread can execute them by calling [execute]. For example:
 * ```
 ...
 * each the ui context so they can use it, but never moving the UI context off the main thread. This
 * effectively runs the tasks on the UI thread without breaking thread confinement.
 *
 ...
 * called, it suspends indefinitely. Multiple concurrent submissions and executions are permitted
 * and quinn is strictly thread-safe, however queue ordering can be non-deterministic in submission
 * race conditions, so external synchronisation can be useful, and only one execute call has effect
 * (with concurrent calls suspending). When Quinn is closed, it completes the present task if one is
 * running, discards all queued tasks, resumes all execute calls, and resumes all suspended
 * submission calls. After closure, no new tasks can be submitted, and attempting to do so throws an
 * exception (unless the closure-safe `try` variants are used); however, new execution calls do not
 * fail and instead return immediately.
 *
 * Exceptions thrown within tasks are handled on the submission side or on the execution side, with
 * the deciding factor being the `errorHandling` argument passed in during task submission.
...
interface Quinn<T> : ObservableClosable {
   ...
   * [Quinn] is already closed when invoked, [InsertionResult.REJECTED_CLOSED] is returned.
   *
   * WARNING: Using the supplied resource (T) outside of [task] is unsupported and not recommended,
   * as the entire purpose of Quinn is accessing thread-bound resources safely. Using the resource
   * outside [task] is likely to cause errors.
   ...
   * closed. Closure allows execution to terminate gracefully and the presently processing task will
   * be completed before tear down.
   *
   * Multiple concurrent invocations of `execute` are permitted and will not fail, but only the
   * first will actively evaluate tasks, and all subsequent calls will suspend. If the active call
   * is cancelled without closure (i.e. coroutine cancellation) then the next waiting call will
   * begin processing (in order of arrival).
   */
