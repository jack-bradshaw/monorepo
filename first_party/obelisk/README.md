...
[design doc](/first_party/backstabnew/design.md).
...
kt_jvm_library_with_dagger(
    ...
    srcs = ["MyComponent.kt"],
    plugins = [
        "//first_party/backstabnew:plugin",
    ],
    deps = [
        "//first_party/backstabnew/annotations:backstab",
        "//first_party/backstabnew/annotations:aggregate_scope",
    ],
)
```
...
graph. The generated module is always named `[ComponentName]_BackstabModule` (e.g.
`NetworkComponent_BackstabModule`).
...
interface AggregateComponent {
  ...
  fun applicationComponent(): OkspApplicationComponent
}
```
...
that installs a different module (e.g. `NetworkComponent_FakeBackstabModule`).
...
```kotlin
@Module
object ThirdPartyComponent_BackstabModule {
  @Provides
  @AggregateScope
