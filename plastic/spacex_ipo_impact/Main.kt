...
fun main() {
  ...
  // Hardcoded sample of realistic data for bottom-tier N100 companies and one mega-cap (AAPL) for
  // comparison
  val marketData =
      listOf(
  ...
  println(
      "SpaceX Float-Adjusted Valuation: \$${spacexFloatAdjustedValuation / 1_000_000_000} Billion")
  println("SpaceX Target Weight in Index: ${String.format("%.2f", spacexWeight * 100)}%")
  println(
      "Total Forced Selling (Nasdaq-100 only): \$${String.format("%.2f", spacexWeight * totalPassiveAum / 1_000_000_000)} Billion")
  println("----------------------------------\n")
  ...
  for (company in marketData) {
    ...
    // We multiply by (1.0 - prePositioningFactor) because hedge funds already caused part of the
    // dip
  ...
  for ((ticker, drop) in sortedResults) {
    ...
    println(
        String.format("%-8s | %-15.1f | %-15.1f | -%.2f%%", ticker, forcedSellM, dailyVolM, drop))
  }
}
