...
This repository uses [Bazel](https://bazel.build) extensively, and the [Bazelisk](https://github.com/bazelbuild/bazelisk) binary is checked into third party for ease of use.

To include the Bazelisk binary in your PATH, run the following from the repo root:

```bash
export PATH="$PWD/third_party/bazelisk:$PATH"
```
